                ...
                crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: Text("Lets find your preferable jobs",style: Theme.of(context).textTheme.bodyLarge,)),
                    ],
                  ),
                   const SizedBox(height: 20,),
                  CustomTextField(
                          ...
                          child: StreamBuilder<List<JobModel>>(
                            ...
                            builder: (context, snapshot) {
                              if (snapshot.connectionState == ConnectionState.waiting) {
                                ...
                                return provider.search ==""?const SizedBox.shrink():
                                ...
                                ListView.separated(
                                  ...
                                  itemBuilder: (cont, index) =>jobModels[index].name.contains(provider.search)?
                                  jobCard(
                                            context: context, data: jobModels[index], provider: provider, index: index):
                                            jobModels[index].description.contains(provider.search)?
                                              jobCard(
                                            context: context, data: jobModels[index], provider: provider, index: index):
                                            jobModels[index].date.toString().contains(provider.search)?  jobCard(
                                            context: context, data: jobModels[index], provider: provider, index: index):const SizedBox.shrink());
